...
const vectorStore = {
    ...
    type: 'vector',
    weight: 1.1,
    query: async (query) => query.toLowerCase().includes('leave') || query.toLowerCase().includes('policy')
        ? [
...
const sqlStore = {
    ...
    type: 'sql',
    weight: 1.4,
    query: async (query) => query.toLowerCase().includes('count') || query.toLowerCase().includes('total') || query.toLowerCase().includes('quarter')
...
const memoryStore = {
    ...
    type: 'memory',
    weight: 1,
    query: async (query) => query.toLowerCase().includes('my') || query.toLowerCase().includes('last time')
